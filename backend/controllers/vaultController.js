...
class VaultController {
  ...
  async getAllVaultItems (req, res, next) {
    try {
      ...
      let processedItems = allItems.map(item => {
        ...
        // Complete the map function that's currently incomplete
        if (isUnlocked) {
          return {
            ...item,
            unlocked: true,
      ...
      const groupedItems = {
        secret_archives: processedItems.filter(
          item => item.category === 'secret_archives'
        ),
        controversy_files: processedItems.filter(
          item => item.category === 'controversy_files'
        ),
        beautiful_mind: processedItems.filter(
          item => item.category === 'beautiful_mind'
        )
      }
      ...
      const stats = {
        ...
        by_category: {
          secret_archives: {
            ...
            unlocked: unlockedItems.filter(
              u => u.category === 'secret_archives'
            ).length
          },
          controversy_files: {
            total: this.vaultItems.controversyFiles.length,
            unlocked: unlockedItems.filter(
              u => u.category === 'controversy_files'
            ).length
          },
          beautiful_mind: {
            total: this.vaultItems.beautifulMindCollection.length,
            unlocked: unlockedItems.filter(u => u.category === 'beautiful_mind')
              .length
          }
        }
      }
  ...
  async markAsRead (req, res, next) {
    try {
      ...
      res.json({
        ...
        data: { itemId, read_at: new Date().toISOString() }
      })
    } catch (error) {
      next(error)
    }
  ...
  /**
   * Check for new vault unlocks based on lesson completion
   * Called by learning controller when lessons are completed
   */
  async checkUnlocks (phase, week, lessonId) {
    try {
      ...
      // Check all vault items for unlock conditions
      const allItems = [
        ...this.vaultItems.secretArchives,
        ...this.vaultItems.controversyFiles,
        ...this.vaultItems.beautifulMindCollection
      ...
      for (const item of allItems) {
        ...
        if (
          this.checkUnlockCondition(item.unlockCondition, phase, week, lessonId)
        ) {
          // Unlock the item!
          await db.run(
            `
                    INSERT INTO vault_unlocks (vault_item_id, category, unlocked_at, unlock_trigger, unlock_phase, unlock_week, unlock_lesson_id)
                    VALUES (?, ?, ?, ?, ?, ?, ?)
                `,
            [
              item.id,
  ...
  /**
   * Manually check and unlock items (for testing/admin)
   */
  async checkAndUnlockItems () {
    try {
      // Get current user progress
      const userProgress = await db.get(`
            SELECT current_phase, current_week, current_lesson_id 
      ...
      // Check for unlocks based on current progress
      return await this.checkUnlocks(
        userProgress.current_phase,
        userProgress.current_week,
        userProgress.current_lesson_id
      )
    } catch (error) {
      console.error('Error in manual unlock check:', error)
      return []
    }
  }
  ...
  checkUnlockCondition (condition, currentPhase, currentWeek, currentLessonId) {
    switch (condition.type) {
      ...
      case 'progress_milestone':
        ...
        return this.checkProgressMilestone(condition, currentPhase, currentWeek)
  ...
  /**
   * Check progress milestone conditions
   */
  async checkProgressMilestone (condition, currentPhase, currentWeek) {
    try {
      ...
      return completionPercentage >= (condition.threshold || 80)
    } catch (error) {
      console.error('Error checking progress milestone:', error)
      return false
    }
  }
   ...
   */
  async searchVaultItems (req, res, next) {
    try {
      const { query, category, unlocked_only } = req.query
      ...
      let items = await this.getAllVaultItems(req, res, next)
      ...
      const filteredItems = items.filter(
        item =>
          ...
          (item.content?.story &&
            item.content.story.toLowerCase().includes(searchTerm))
      )
  ...
  async getVaultAnalytics (req, res, next) {
    try {
      ...
      const totalItems =
        this.vaultItems.secretArchives.length +
      ...
      res.json({
        ...
        data: {
          overview: {
            ...
            unlock_percentage:
              totalItems > 0
                ? ((overallStats.total_unlocked / totalItems) * 100).toFixed(1)
                : 0,
            read_percentage:
              overallStats.total_unlocked > 0
                ? (
                    (overallStats.total_read / overallStats.total_unlocked) *
                    100
                  ).toFixed(1)
                : 0
          },
  ...
  async getUnlockTimeline (req, res, next) {
    try {
      ...
      const enrichedTimeline = timeline.map(unlock => {
        const vaultItem = allItems.find(
          item => item.id === unlock.vault_item_id
        )
        return {
          ...unlock,
    ...
    try {
      ...
      const upcomingRewards = allItems
        ...
        .map(item => ({
          ...
          icon: item.icon,
          unlock_condition: item.unlockCondition,
          estimated_unlock: this.estimateUnlockTime(
  ...
  estimateUnlockTime (condition, currentProgress) {
    if (condition.type === 'lesson_complete') {
      const weeksAway =
        (condition.phase - currentProgress.current_phase) * 12 +
