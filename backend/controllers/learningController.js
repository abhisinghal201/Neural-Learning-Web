...
class LearningController {
    ...
    async getProgress(req, res, next) {
        try {
            ...
            const stats = {
                ...
                completed_lessons: progress.filter(p => ['completed', 'mastered'].includes(p.status)).length,
                ...
                completion_rate: progress.length > 0 
                    ? (progress.filter(p => ['completed', 'mastered'].includes(p.status)).length / progress.length * 100).toFixed(1)
                    : 0
            };
    ...
    async updateProgress(req, res, next) {
        try {
            ...
            const updateData = {
                ...
                completion_percentage: completion_percentage !== undefined ? completion_percentage : currentLesson.completion_percentage,
                time_spent_minutes: (currentLesson.time_spent_minutes || 0) + (time_spent_minutes || 0),
                mastery_score: mastery_score !== undefined ? mastery_score : currentLesson.mastery_score,
                notes: notes !== undefined ? notes : currentLesson.notes,
                updated_at: new Date().toISOString()
            };
            
            // Set completion date if marking as completed/mastered
            if (['completed', 'mastered'].includes(status) && !currentLesson.completed_at) {
            ...
            if (['completed', 'mastered'].includes(status) && currentLesson.status !== status) {
            ...
            await this.checkVaultUnlocks(currentLesson.phase, currentLesson.week, lessonId);
    ...
    async startSession(req, res, next) {
        try {
            ...
            const validTypes = ['math', 'coding', 'visual_projects', 'real_applications'];
            ...
            const sessionData = {
                ...
                completed_goals: JSON.stringify(goals || []),
                session_notes: ''
            };
            
            const result = await db.run(`
                INSERT INTO daily_sessions (session_date, session_type, start_time, energy_level, completed_goals, session_notes)
                VALUES (?, ?, ?, ?, ?, ?)
            `, [
    ...
    async endSession(req, res, next) {
        try {
            ...
            const duration = Math.round((new Date(endTime) - new Date(session.start_time)) / 60000); // minutes
            
            await db.run(`
                UPDATE daily_sessions 
                SET end_time = ?, duration_minutes = ?, focus_score = ?, session_notes = ?, completed_goals = ?
                WHERE id = ?
            `, [
                endTime,
                duration,
                ...
                JSON.stringify(completed_goals || JSON.parse(session.completed_goals)),
                sessionId
            ]);
            
            // Award persistence points for completing sessions
            await this.awardSkillPoints('persistence', 'session_completed', sessionId.toString());
            
            // Update user profile with session time
            await this.updateUserProfile(duration);
    ...
    async getTodaySessions(req, res, next) {
        try {
            ...
            const sessionTypes = ['math', 'coding', 'visual_projects', 'real_applications'];
            const usedTypes = sessions.map(s => s.session_type);
            const nextRecommended = sessionTypes.find(type => !usedTypes.includes(type)) || sessionTypes[0];
    ...
    async submitQuest(req, res, next) {
        try {
            ...
            await this.awardQuestSkillPoints(quest_type, difficulty_level, status, quest_id);
            
            // Generate AI mentor feedback (placeholder for now)
            const mentorFeedback = await this.generateMentorFeedback(code_solution, execution_result, quest_type);
    ...
    async submitReview(req, res, next) {
        try {
            ...
            const nextReviewDate = moment().add(newInterval, 'days').format('YYYY-MM-DD');
            
            await db.run(`
                UPDATE spaced_repetition 
                SET interval_days = ?, difficulty_factor = ?, repetition_count = repetition_count + 1,
                    next_review_date = ?, last_reviewed_at = ?, quality_score = ?
                WHERE concept_id = ?
            `, [
                newInterval,
                newDifficulty,
    ...
    async awardSkillPoints(category, reason, relatedId) {
        const pointsMap = {
            'theory': { completed: 10, mastered: 25 },
            'math': { completed: 15, mastered: 30 },
            'visual': { completed: 10, mastered: 20 },
            'coding': { completed: 20, mastered: 40 },
            'persistence': { session_completed: 5 }
        };
        
        const points = pointsMap[category]?.[reason] || 5;
        
        await db.run(`
            INSERT INTO skill_points (category, points_earned, reason, related_lesson_id)
            VALUES (?, ?, ?, ?)
        `, [category, points, `${reason} achievement`, relatedId]);
    }
    ...
    async awardQuestSkillPoints(questType, difficulty, status, questId) {
        ...
        const points = (basePoints[questType] || 10) * difficulty * (status === 'mastered' ? 1.5 : 1);
    ...
    async checkVaultUnlocks(phase, week, lessonId) {
        // This will be implemented when we connect to the vault system
        // For now, it's a placeholder
        console.log(`Checking vault unlocks for Phase ${phase}, Week ${week}, Lesson ${lessonId}`);
    }
    ...
    async generateMentorFeedback(code, result, questType) {
        ...
        const feedbackTemplates = {
            'coding_exercise': 'Great work on the implementation! Consider optimizing for better performance.',
            'implementation_project': 'Excellent project completion! Your code structure shows good understanding.',
            'theory_quiz': 'Well done! Your theoretical understanding is solid.',
            'practical_application': 'Impressive practical application! Consider exploring edge cases.'
        };
        
        return feedbackTemplates[questType] || 'Good effort! Keep practicing to improve your skills.';
    }
