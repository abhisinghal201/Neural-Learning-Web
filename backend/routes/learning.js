  ...
  try {
    ...
    const completionPercentage =
      overallStats.total_lessons > 0
        ? Math.round(
            (overallStats.completed_lessons / overallStats.total_lessons) * 100
          )
        : 0
...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      await learningController.submitQuest(req, res, next)
    } catch (error) {
      next(error)
    }
    try {
      ...
      if (status === 'completed' || status === 'mastered') {
        ...
        await db.run(
          ...
          [
            'theory',
            pointsEarned,
            `Completed lesson: ${updatedLesson.lesson_title}`,
            id
          ]
        )
      }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      const updates = {
        status:
          mastery_score && mastery_score >= 0.8 ? 'mastered' : 'completed',
...
// GET /api/v1/learning/review - Get items due for review
router.get('/review', async (req, res, next) => {
  try {
    ...
    const reviewItems = await db.query(
      `
            SELECT * FROM spaced_repetition 
            WHERE next_review_date <= ? AND is_active = 1
...
// POST /api/v1/learning/review/:conceptId - Submit review result
router.post(
  '/review/:conceptId',
  [
    param('conceptId').notEmpty().withMessage('Concept ID is required'),
    body('quality_score')
      .isInt({ min: 0, max: 5 })
  ...
  async (req, res, next) => {
    try {
      const learningController = require('../controllers/learningController')
      await learningController.submitReview(req, res, next)
    } catch (error) {
      next(error)
    }
  }
    ...
    try {
      ...
      await db.run(
        ...
        [
          'programming',
          pointsEarned,
          `${status} quest: ${quest_title}`,
          quest_id
        ]
      )

      // Get the created quest
      const createdQuest = await db.get(
...
// POST /api/v1/learning/quests/submit - Submit quest completion
router.post(
  '/quests/submit',
  [
    body('quest_id').notEmpty().withMessage('Quest ID is required'),
...
// POST /api/v1/learning/sessions/start - Start new learning session
router.post(
  '/sessions/start',
  [
    body('session_type').isIn([
  ...
  async (req, res, next) => {
    try {
      const learningController = require('../controllers/learningController')
      await learningController.startSession(req, res, next)
    } catch (error) {
      next(error)
    }
  }
...
// PUT /api/v1/learning/sessions/:id/end - End learning session
router.put(
  ...
  [
    ...
    body('completed_goals').optional().isArray()
  ],
  handleValidationErrors,
  async (req, res, next) => {
    try {
      const learningController = require('../controllers/learningController')
      await learningController.endSession(req, res, next)
    } catch (error) {
      next(error)
    }
  }
...
// GET /api/v1/learning/sessions/today - Get today's sessions
router.get('/sessions/today', async (req, res, next) => {
  try {
    const today = new Date().toISOString().split('T')[0]
...
router.put(
  ...
  [
    ...
    body('end_time')
      .optional()
      .isISO8601()
      .withMessage('End time must be valid ISO date'),
    body('focus_score').optional().isInt({ min: 1, max: 10 }),
    body('energy_level').optional().isInt({ min: 1, max: 10 }),
...
router.get('/next-lesson', async (req, res, next) => {
  try {
    ...
    if (!nextLesson) {
      return res.json({
        message:
          'Congratulations! You have completed your Neural Odyssey journey!',
        completed: true
      })
    }
