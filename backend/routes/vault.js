...
router.get('/items', async (req, res, next) => {
  try {
    const vaultController = require('../controllers/vaultController')
    await vaultController.getAllVaultItems(req, res, next)
  } catch (error) {
    next(error)
  }
  try {
    ...
    res.json({
      ...
      data: {
        ...
        categories: {
          secret_archives: enrichedItems.filter(
            i => i.category === 'secret_archives'
          ).length,
          controversy_files: enrichedItems.filter(
            i => i.category === 'controversy_files'
          ).length,
          beautiful_mind: enrichedItems.filter(
            i => i.category === 'beautiful_mind'
          ).length
        }
      }
    })
    ...
    try {
      ...
      res.json({
        ...
        data: {
          ...
          icon: vaultItem.icon,
          category_display: unlockedItem.category
            .split('_')
            .map(w => w.charAt(0).toUpperCase() + w.slice(1))
            .join(' ')
        }
...
router.get('/stats', async (req, res, next) => {
  try {
    ...
    const totalAvailable =
      (vaultItemsData.secretArchives?.length || 0) +
...
// GET /api/v1/vault/items/:itemId - Get specific vault item
router.get(
  '/items/:itemId',
  [param('itemId').notEmpty().withMessage('Item ID is required')],
  handleValidationErrors,
  async (req, res, next) => {
    try {
      const vaultController = require('../controllers/vaultController')
      await vaultController.getVaultItem(req, res, next)
    } catch (error) {
      next(error)
    }
  }
...
// POST /api/v1/vault/items/:itemId/read - Mark item as read
router.post(
  '/items/:itemId/read',
  [param('itemId').notEmpty().withMessage('Item ID is required')],
  handleValidationErrors,
  async (req, res, next) => {
    try {
      const vaultController = require('../controllers/vaultController')
      await vaultController.markAsRead(req, res, next)
    } catch (error) {
      next(error)
    }
  }
...
// POST /api/v1/vault/items/:itemId/rate - Rate vault item
router.post(
  '/items/:itemId/rate',
  [
    param('itemId').notEmpty().withMessage('Item ID is required'),
  ...
  async (req, res, next) => {
    try {
      const vaultController = require('../controllers/vaultController')
      await vaultController.rateItem(req, res, next)
    } catch (error) {
      next(error)
    }
  }
...
// GET /api/v1/vault/analytics - Get vault analytics
router.get('/analytics', async (req, res, next) => {
  try {
    const vaultController = require('../controllers/vaultController')
    await vaultController.getVaultAnalytics(req, res, next)
  } catch (error) {
    next(error)
  }
})

// GET /api/v1/vault/timeline - Get unlock timeline
router.get('/timeline', async (req, res, next) => {
  try {
    const vaultController = require('../controllers/vaultController')
    await vaultController.getUnlockTimeline(req, res, next)
  } catch (error) {
    next(error)
  }
})

// GET /api/v1/vault/upcoming - Get upcoming rewards
router.get('/upcoming', async (req, res, next) => {
  try {
    const vaultController = require('../controllers/vaultController')
    await vaultController.getUpcomingRewards(req, res, next)
  } catch (error) {
    next(error)
  }
})

// POST /api/v1/vault/check-unlocks - Manual unlock check
router.post(
  '/check-unlocks',
  [
    body('phase').optional().isInt({ min: 1, max: 4 }),
    body('week').optional().isInt({ min: 1, max: 12 }),
  ...
  async (req, res, next) => {
    try {
      const vaultController = require('../controllers/vaultController')
      await vaultController.triggerUnlockCheck(req, res, next)
    } catch (error) {
      next(error)
    }
  }
...
router.get(
  ...
  [
    ...
    query('category')
      .optional()
      .isIn(['secret_archives', 'controversy_files', 'beautiful_mind']),
    query('unlocked_only').optional().isBoolean()
  ],
  handleValidationErrors,
  async (req, res, next) => {
    try {
      const vaultController = require('../controllers/vaultController')
      await vaultController.searchVaultItems(req, res, next)
    } catch (error) {
      next(error)
    }
  }
...
// POST /api/v1/vault/reload - Reload vault items (development)
router.post('/reload', async (req, res, next) => {
  try {
    const vaultController = require('../controllers/vaultController')
    await vaultController.reloadVaultItems(req, res, next)
  } catch (error) {
    next(error)
  }
})
