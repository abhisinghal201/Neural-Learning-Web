...
class PortfolioExporter {
  ...
  generateMarkdownPortfolio (data) {
    ...
    markdown += `| **Total Study Time** | ${this.formatDuration(
      analytics.total_study_time
    )} |\n`
    markdown += `| **Average Time per Lesson** | ${this.formatDuration(
      Math.round(analytics.avg_time_per_lesson)
    )} |\n`
    ...
    if (vault.length > 0) {
      ...
      Object.entries(vaultByCategory).forEach(([category, items]) => {
        ...
        markdown += `### ${categoryEmoji} ${category
          .split('_')
        ...
        items.forEach(item => {
          markdown += `- **${
            item.vault_item_id
          }** - Unlocked ${this.formatDateTime(item.unlocked_at)}\n`
          if (item.user_rating) {
    ...
    if (lessons.length > 0) {
      ...
      Object.entries(lessonsByPhase).forEach(([phase, phaseLessons]) => {
        ...
        markdown += `### Phase ${phase}: ${
          phaseNames[phase] || 'Advanced Learning'
        }\n\n`
        ...
        Object.entries(lessonsByWeek).forEach(([week, weekLessons]) => {
          ...
          weekLessons.forEach(lesson => {
            ...
            markdown += `  - Completed: ${this.formatDateTime(
              lesson.completed_at
            )}\n`
            markdown += `  - Time spent: ${this.formatDuration(
              lesson.time_spent_minutes
            )}\n`
            if (lesson.mastery_score) {
              markdown += `  - Mastery: ${Math.round(
                lesson.mastery_score * 100
              )}%\n`
            }
            if (lesson.notes) {
    ...
    if (quests.length > 0) {
      ...
      Object.entries(questsByPhase).forEach(([phase, phaseQuests]) => {
        ...
        phaseQuests.forEach(quest => {
          ...
          markdown += `- **Time to Complete:** ${this.formatDuration(
...
<head>
    ...
    <style>
        ...
        h1, h2, h3 { color: #00d4ff; border-bottom: 2px solid #333; padding-bottom: 0.5rem; }
        table { width: 100%; border-collapse: collapse; margin: 1rem 0; }
        th, td { border: 1px solid #333; padding: 0.8rem; text-align: left; }
        th { background: #1a1a1a; color: #00d4ff; }
        tr:nth-child(even) { background: #1a1a1a; }
        code { background: #1a1a1a; padding: 0.2rem 0.4rem; border-radius: 3px; color: #ff6b9d; }
        pre { background: #1a1a1a; padding: 1rem; border-radius: 8px; overflow-x: auto; border-left: 4px solid #00d4ff; }
        .emoji { font-size: 1.2em; }
        .stat-card { background: #1a1a1a; padding: 1rem; border-radius: 8px; margin: 0.5rem 0; border: 1px solid #333; }
        .progress-bar { background: #333; height: 8px; border-radius: 4px; overflow: hidden; }
        .progress-fill { background: linear-gradient(90deg, #00d4ff, #8b5cf6); height: 100%; transition: width 0.3s ease; }
    </style>
</head>
<body>
  ...
  async exportPortfolio () {
    console.log(
      `📊 Exporting portfolio (Phase: ${options.phase}, Format: ${options.format})...`
    )
    ...
    const data = {
      ...
      analytics: await this.getLearningAnalytics()
    }
    ...
    console.log(
      `   Total Study Time: ${this.formatDuration(
        data.analytics.total_study_time
      )}`
    )
  }
}
